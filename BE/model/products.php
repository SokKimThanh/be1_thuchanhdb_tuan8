...
class ArrayListSanPham
{
    ...
    public function findById($id)
    {
        foreach ($this->list as $key => $value) {
            if ($value['id'] == $id) {
                return new Product($value['id'], $value['name'], $value['manu_id'], $value['type_id'], $value['price'], $value['pro_image'], $value['description'], $value['feature'], $value['created_at']);
            }
        }
        return null;
    }
